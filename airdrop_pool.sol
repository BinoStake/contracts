    ...
    AccessControlUpgradeable
{
    ...
    uint256 public constant TOTAL_REWARDS = 43200000000000000000; // total priovided bsBNB amount - 43.2 bsBNB
    ...
    function initialize(
        ...
        bsBnbPerSecond = 1e13; // bsBNB reward amount persecodn - 0.00001
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 tokenSupply = IERC20Upgradeable(pool.token).balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bsBnbReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accBsBNBPerToken = accBsBNBPerToken.add(_bsBnbReward.div(tokenSupply));
        }
        return user.amount.mul(accBsBNBPerToken).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bsBnbReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accBsBNBPerToken = pool.accBsBNBPerToken.add(_bsBnbReward.div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accBsBNBPerToken).sub(user.rewardDebt);
            if (_pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accBsBNBPerToken);
